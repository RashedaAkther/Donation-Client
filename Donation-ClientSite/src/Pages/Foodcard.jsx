...
const Foodcard = ({ food }) => {
  const handleViewDetailsClick = async () => {
    try {
      ...
      await axios.post("https://foods-donations-assignment.vercel.app/food/click", {
        foodId: food?._id,
      });
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="h-[550px] ">
      <div className="max-w-xs rounded-md shadow-xl dark:bg-gray-900 dark:text-gray-100">
        ...
        <Link onClick={handleViewDetailsClick} to={`/featureFood/${_id}`}>
          <button className="flex items-center  justify-center w-full p-3 mb-10 font-semibold tracki rounded-md  btn bg-green-600 text-white hover:bg-green-900 ">
            Details
          </button>
        </Link>
      </div>
